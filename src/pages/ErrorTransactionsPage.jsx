...
import { fetchErrorTransactions } from '../services/api';

const ErrorTransactionsPage = () => {
    const [transactions, setTransactions] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const fetchData = async () => {
            const data = await fetchErrorTransactions();
        ...
        <Box sx={{ display: 'flex' }}>
            ...
            <Box sx={{ flexGrow: 1 }}>
                ...
                <Box sx={{ padding: '30px', marginTop: '70px', marginBottom: '60px' }}>
                    {/* Display Title - Error Transactions */}
                    <Box sx={{ marginBottom: '30px' }}>
                        <Typography variant="h4" component="h1" sx={{ fontWeight: 'bold', color: '#fc211a' }}>
                            Error Transactions
                        </Typography>
                    </Box>
                    ...
                    <Grid container spacing={3}>
                        <KPI title="Total Error Transactions" value={transactions.length} color="#FF5722" />
...
export default ErrorTransactionsPage;
